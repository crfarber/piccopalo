...
// MARK: - Swipeable row met vierkante actieknoppen

private struct SwipeableTimelineRow<Content: View>: View {
    let id: String
    ...
    private var openWidth: CGFloat { buttonWidth * 2 }

    var body: some View {
        ZStack(alignment: .trailing) {
            HStack(spacing: 0) {
                actionButton(icon: "pencil", background: Theme.Colors.cream, foreground: Theme.Colors.background) {
                    close()
                    onEdit()
                }
